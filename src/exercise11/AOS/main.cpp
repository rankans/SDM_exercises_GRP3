...
// Distributed Worker for Partitioning and Shuffle
class DistributedWorker
{
public:
    int worker_id;
    r1_distributed_aos r1_local;
    r2_distributed_aos r2_local;
    DistributedWorker(int id) : worker_id(id) {}
    void generate_local_data(size_t records_per_worker, std::int64_t key_range)
    {
        std::cout << "Worker " << worker_id << ": Generating local data...\n";
        r1_local = r1_distributed_aos::generate_relation(records_per_worker, key_range);
        r2_local = r2_distributed_aos::generate_relation(records_per_worker, key_range);
        std::cout << "Worker " << worker_id << ": Generated R1(" << r1_local.size()
                  << ") R2(" << r2_local.size() << ")\n";
    }
};
// Correctness verification: Check that all tuples with same join key end up on same worker
void verify_correctness(const std::vector<DistributedWorker> &workers)
{
    ...
    // Check R1 key distribution
    for (size_t worker_i = 0; worker_i < workers.size(); ++worker_i)
    {
        std::unordered_set<std::int64_t> worker_keys;
        for (const auto &record : workers[worker_i].r1_local.records())
        {
            std::int64_t key = record.column_a();
    ...
    // Check R2 key distribution
    for (size_t worker_i = 0; worker_i < workers.size(); ++worker_i)
    {
        std::unordered_set<std::int64_t> worker_keys;
        for (const auto &record : workers[worker_i].r2_local.records())
        {
            std::int64_t key = record.column_a();
            worker_keys.insert(key);
    ...
    {
        std::cout << "CORRECTNESS VERIFIED: All tuples with same join key on same worker\n";
    }
    else
    {
        std::cout << "CORRECTNESS FAILED: Key distribution violations found\n";
    }
}
// Simulate distributed partitioning and shuffle
void simulate_distributed_partitioning_shuffle(std::vector<DistributedWorker> &workers)
{
    size_t num_workers = workers.size();
    std::cout << "\n=== Phase 1: Local Partitioning ===\n";
    // Store all partitions from all workers
    std::vector<std::vector<std::vector<r1_distributed_aos::record_type>>> r1_all_partitions(num_workers);
    std::vector<std::vector<std::vector<r2_distributed_aos::record_type>>> r2_all_partitions(num_workers);
    ...
    for (size_t worker_i = 0; worker_i < num_workers; ++worker_i)
    {
        ...
        {
            std::cout << "P" << p << "(" << r1_all_partitions[worker_i][p].size() << ") ";
        }
        std::cout << "\n";
        std::cout << "Worker " << worker_i << " R2 partitions: ";
        for (size_t p = 0; p < num_workers; ++p)
        {
            std::cout << "P" << p << "(" << r2_all_partitions[worker_i][p].size() << ") ";
        }
        std::cout << "\n";
    }
    ...
    for (size_t receiver_worker = 0; receiver_worker < num_workers; ++receiver_worker)
    {
        ...
        for (size_t sender_worker = 0; sender_worker < num_workers; ++sender_worker)
        {
            ...
            workers[receiver_worker].r1_local.shuffle_phase_receive(r1_partition);
            workers[receiver_worker].r2_local.shuffle_phase_receive(r2_partition);
            total_r1_received += r1_partition.size();
            total_r2_received += r2_partition.size();
        }
...
int main()
{
    std::cout << "=== Exercise 11: Distributed Partitioning and Shuffle (AOS) ===\n\n";
    ...
    for (size_t num_workers : worker_counts)
    {
        std::cout << "\n"
                  << std::string(60, '=') << "\n";
        std::cout << "TESTING WITH " << num_workers << " WORKERS\n";
        std::cout << std::string(60, '=') << "\n";
        // Create distributed workers
        std::vector<DistributedWorker> workers;
        ...
        simulate_distributed_partitioning_shuffle(workers);

        // Verify correctness
        verify_correctness(workers);
    }
    std::cout << "\n=== Summary ===\n";
    std::cout << "Demonstrated distributed partitioning and shuffle with:\n";
    ...
    std::cout << "4. Schema: R1(A,B) and R2(A,B) where A=join key, B=payload\n";
