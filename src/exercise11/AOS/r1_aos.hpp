...
{
    // Relation R1 with distributed partitioning capabilities
    // Schema: R1(A,B) where A=join key, B=payload
    class r1_distributed_aos;
    class r1_distributed_aos_record
    {
    private:
        std::int64_t _column_a; // Join key (column A)
        std::string _column_b;  // Payload (column B)
    public:
        r1_distributed_aos_record(std::int64_t column_a, std::string column_b);
        const std::int64_t &column_a() const noexcept { return _column_a; }
        const std::string &column_b() const noexcept { return _column_b; }
        friend class r1_distributed_aos;
    };
    class r1_distributed_aos
    {
    public:
        using record_type = r1_distributed_aos_record;
    ...
    public:
        r1_distributed_aos() = default;
        // Generate initial relation data for worker
        static r1_distributed_aos generate_relation(size_t X, std::int64_t Y);
