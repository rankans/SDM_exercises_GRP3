...
{
    // Relation R2 with Structure of Arrays (columnar) layout and distributed partitioning
    // Schema: R2(A,B) where A=join key, B=payload
    class r2_distributed_soa
    {
    private:
    ...
    public:
        r2_distributed_soa() = default;
        // Generate initial relation data for worker
        static r2_distributed_soa generate_relation(size_t X, std::int64_t Y);
