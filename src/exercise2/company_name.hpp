...
private:
    company_name_record(std::int64_t id,
                        ...
                        std::string md5sum)
        : _id(id), _name(std::move(name)), _country_code(std::move(country_code)), _imdb_id(imdb_id), _name_pcode_nf(std::move(name_pcode_nf)), _name_pcode_sf(std::move(name_pcode_sf)), _md5sum(std::move(md5sum))
    {
    }
